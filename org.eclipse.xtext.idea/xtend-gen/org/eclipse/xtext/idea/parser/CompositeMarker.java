...
public class CompositeMarker {
  ...
  protected CreateElementType withUserData(final IElementType elementType) {
    final CreateElementType.CreateCallback _function = (ASTNode it) -> {
      it.<Integer>putUserData(IASTNodeAwareNodeModelBuilder.LOOK_AHEAD_KEY, Integer.valueOf(this.lookAhead));
      it.<Boolean>putUserData(IASTNodeAwareNodeModelBuilder.HAS_SEMANTIC_ELEMENT_KEY, Boolean.valueOf(this.hasSemanticElement));
    };
