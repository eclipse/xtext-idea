...
import static org.eclipse.xtext.parsetree.reconstr.idea.parser.antlr.internal.PsiInternalSerializationBug269362TestLanguageParser.*;

@Singleton public class SerializationBug269362TestLanguageTokenTypeProvider implements TokenTypeProvider {

	private static final String[] TOKEN_NAMES = new PsiInternalSerializationBug269362TestLanguageParser(null).getTokenNames();

	private static final IElementType[] tokenTypes = new IElementType[TOKEN_NAMES.length];
	
	static {
		for (int i = 0; i < TOKEN_NAMES.length; i++) {
			tokenTypes[i] = new IndexedElementType(TOKEN_NAMES[i], i, SerializationBug269362TestLanguageLanguage.INSTANCE);
		}
	}
