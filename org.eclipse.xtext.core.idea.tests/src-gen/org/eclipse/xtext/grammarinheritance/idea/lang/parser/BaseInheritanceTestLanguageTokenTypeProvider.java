...
import static org.eclipse.xtext.grammarinheritance.idea.parser.antlr.internal.PsiInternalBaseInheritanceTestLanguageParser.*;

@Singleton public class BaseInheritanceTestLanguageTokenTypeProvider implements TokenTypeProvider {

	private static final String[] TOKEN_NAMES = new PsiInternalBaseInheritanceTestLanguageParser(null).getTokenNames();

	private static final IElementType[] tokenTypes = new IElementType[TOKEN_NAMES.length];
	
	static {
		for (int i = 0; i < TOKEN_NAMES.length; i++) {
			tokenTypes[i] = new IndexedElementType(TOKEN_NAMES[i], i, BaseInheritanceTestLanguageLanguage.INSTANCE);
		}
	}
